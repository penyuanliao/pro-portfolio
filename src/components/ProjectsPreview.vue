...
<template>
  <section class="project-section relative z-10 flex flex-col items-center py-32">
    ...
    <!-- 視差卡片容器 -->
    <div class="project-list flex flex-col gap-24 md:gap-40 w-full max-w-5xl px-4">
      <div
        v-for="(project, index) in projects"
        :key="index"
        class="parallax-card w-full flex justify-center"
    ...
    <!-- 背景裝飾物 (可強化視差感) -->
    <div class="absolute top-1/4 left-10 h-64 w-64 rounded-full bg-blue-400/10 blur-3xl"></div>
    <div class="absolute bottom-1/4 right-10 h-96 w-96 rounded-full bg-purple-400/10 blur-3xl"></div>
  </section>
</template>
